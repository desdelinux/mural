        ...
        for field_name, swift_line in required:
            if field_name not in kotlin_set:
                failures.append(format_failure(
                    ...
                    kotlin_path, kotlin_line, swift_path, swift_line))
        for field_name in sorted(set(swift_names) - required_names - kotlin_set):
            failures.append(format_failure(
                'archive-fields', f'{struct_name}.{field_name} is optional in Swift but missing from the Kotlin data class, so an exported value would be dropped',
                kotlin_path, kotlin_line, swift_path, struct_line))
