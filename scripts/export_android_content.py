...
def generate(core):
    ...
    for path in language_files(core):
        ...
        focus_values = swift_strings(focus)
        if len(focus_values) != 6:
            raise SystemExit(f'{path}: teachingFocus must contain exactly six entries, found {len(focus_values)}.')
        args.append('        teachingFocus = listOf(' + ', '.join(map(quoted, focus_values)) + ')')
        overrides = []
    ...
    lines += meaning_languages((core / 'Languages/LanguageModule.swift').read_text())
    lines += ['}', '']
    return '\n'.join(lines)
